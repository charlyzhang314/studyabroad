...
function initInterest(){
	...
	$(document).scroll(function (){
		...
		timer = window.setTimeout(function() {
			...
			var banksSectionY = $('#section-banks-slogan').offset().top - 30;
...
function updateCardsSection(data) {
	...
	data.forEach(function(card, index) {
		...
		$ele.find(".know_btn").attr("href", card.link);
...
function onClickOffer(btn) {
    ...
    if(bankName){
    	link = "offer-detail.php?id="+offerId+"&bank=" + bankName;
    }else if(bankId){
    	link = "offer-detail.php?id="+offerId+"&bankid=" + bankId;
    }else{
    	link = "offer-detail.php?id="+offerId;
    }
...
function onClickOfferDetailApply(btn) {
	...
	if (isBank) {
		if (bank && bankdata[bank] && bankdata[bank].cards) {
			...
			changeLocWithDelay("cards.php?bankid="+bank_id);
		}else {
