...
var bankIdvalues = [
	"boc", "boc_debit", "abc", "cgb", "spdb", "icbc",
	"ccb", "pab", "ceb", "cmbc", "bos", "scb",
	"bocom", "cmb", "cib", "citic", "ccbji",
	"abc_debit", "bob", "bon"
];
...
function updateBannersSection(data, country, status) {
	...
	if(isBank){
		if(bank){
			...
			// if(location.hash.split("-")[2] == 'preparation'){
			// 	bannerdata.unshift(newFirstBanner[0]);
			// }else if(location.hash.split("-")[2] == 'pretrip'){
			// 	bannerdata.unshift(newFirstBanner[1]);
			// }else if(location.hash.split("-")[2] == 'destination'){
			// 	bannerdata.unshift(newFirstBanner[2]);
			// }else{
			// 	console.log("nonono")
			// }
		}else if(bankid){
	...
	bannerdata.forEach(function(banner, index) {
		...
		if (banner.link) {
			var isLocal = banner.link.lastIndexOf("http", 0) !== 0;		//  是否站内链接 true false
			console.log(bank)
			if (isBank && bank) {    // 页面url带bank参数
				//banner.link = updateQueryStringParameter(banner.link, BANK_PARAM, bank);
				if (!isLocal || banner.link.indexOf("php") > -1) {
					//banner.link = updateQueryStringParameter(banner.link, BANK_PARAM, bank);
				...
				// 	banner.link = updateQueryStringParameter(banner.link, BANK_ID_PARAM, bankdata[bank].bankid);
				// }
			}else if(isBank && bankid){		// 页面url带bankid参数
				if(banner.link != "https://visaoffer.yijiuplus.com/index/selected?bankid=visa_wechat"){
					if(banner.link.indexOf("http") > -1){
						banner.link = banner.link + "?bankid=" + bankid;
					}
				}
				console.log(2222222)
			}
	...
	if (isBank && bank) {
		if (bankdata[bank] && bankdata[bank].cards) {
			...
			data.forEach(function(card, index) {
				...
				$ele.attr("data-pic", card.pop);
				$ele.attr("data-link", card.link);
				$ele.attr("data-id", card.id);
				$ele.attr("data-bank", card.bankname);
				$ele.attr("data-scroll", card.scroll);
...
function updateBenefitsSection(data, country, status) {
	...
	var no_medical = getUrlParameter(NO_MEDICAL_PARAM);
	var bank = getCookie(C_BANK);
	var bank_id = getUrlParameter('bankid');
	...
	data.forEach(function(category) {
		if (category.category && category.category == "explore-more") {
			...
			category.benefits.forEach(function(benefit, index) {
				...
				}else if(isBank && bank_id){
					benefit.link = benefit.link + "&bankid=" + bank_id
				}
